...
contract ArgonLottery is VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    modifier isContractActive() {
        require(lotteryStatus, "Lottery is not active");
        _;
    }
    ...
    function changeActive(bool _status) external nonReentrant onlyOwner {
        lotteryStatus = _status;
    }

    function buyTicket(uint256 _ticketAmount)
        external
        isContractActive
        nonReentrant
    {
        ...
        token.transferFrom(
            msg.sender,
            address(this),
            _ticketAmount.mul(tokenPerTicket)
        );
        rewardTokenAmount = rewardTokenAmount.add(
            _swap(_ticketAmount.mul(tokenPerTicket))
        );
    ...
    function emergencyWithdrawLinkTokens() external nonReentrant onlyOwner {
        require(block.timestamp >= endTime);
        require(linkToken.balanceOf(address(this)) > 0);
        linkToken.safeTransfer(msg.sender, linkToken.balanceOf(address(this)));
    }

    function _swap(uint256 _amountIn) private returns (uint256) {
        //first we need to transfer the amount in tokens from the msg.sender to this contract
        //this contract will have the amount of in tokens

        //next we need to allow the uniswapv2 router to spend the token we just sent to this contract
        //by calling IERC20 approve you allow the uniswap contract to spend the tokens in this contract
        token.safeApprove(UNISWAP_V2_ROUTER, _amountIn);
    ...
    function _getAmountOutMin(uint256 _amountIn)
        private
        view
        returns (uint256)
    {
